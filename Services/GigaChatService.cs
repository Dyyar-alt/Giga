    ...
    public class GigaChatService
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            try
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"⚠️ AndroidHttpClientHandler ошибка: {ex.Message}");
                }
#endif
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"⚠️ IgnoreTLS ошибка: {ex.Message}");
                }
        ...
        public async Task<string> SendMessageAsync(string message)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("⚠️ Client not initialized, попытка переинициализации...");
            ...
            catch (Exception ex)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("⚠️ Ошибка сертификата, пробуем переинициализацию...");
        ...
        public async Task<byte[]?> DownloadImageAsync(string fileId)
        {
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"⬇️ Скачивание через SDK: {fileId}");
            ...
            catch (Exception ex)
            {
                ...
                if (ex.Message.Contains("certificate") || ex.Message.Contains("SSL") || ex.Message.Contains("TLS") || ex.Message.Contains("Authentication"))
                {
                    System.Diagnostics.Debug.WriteLine("⚠️ Ошибка сертификата при скачивании, пробуем переинициализацию...");
                    _isInitialized = false;
                    await InitializeAsync();
